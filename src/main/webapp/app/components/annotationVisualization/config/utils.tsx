...
export function getCategoricalAlteration(
  alteration: string
): string | undefined {
...
export function defaultSortMethod(
  a: string | ONCOGENICITY | number,
  b: string | ONCOGENICITY | number
): number {
...
export const OncoKBLevelIcon: React.FunctionComponent<{
  ...
  return withDescription && level ? (
    <LevelWithDescription level={level as LEVELS}>
      {oncokbIcon}
    </LevelWithDescription>
  ) : (
    <>{oncokbIcon}</>
...
export function getDefaultColumnDefinition(
  ...
  | {
      ...
      Cell?:
        | React.FC<{ original: AnnotationImplication }>
        | React.FC<{ original: TreatmentImplication }>;
...
export const generatePDF = async (
  ...
  try {
    ...
    const groupedTreatmentData = processedTreatmentData.reduce<
      GroupedTreatmentData
    >((acc, row) => {
      const { alterationType } = row;
      if (!acc[alterationType]) {
        acc[alterationType] = [];
        ...
        <table border="0.5" cellspacing="0" cellpadding="5" style="width: 100%; border-collapse: collapse; border-color: black;">
          ...
          <tbody>
            ${Object.entries(groupedData)
              ...
              ${rows
                .map((row, index) => {
                  const levelClass = LevelOfEvidenceToClassnames(row.level);
                  const oncogenicityClass = OncogenicityToClassnames(
                    row.oncogenicity
                  );
                  const backgroundColor =
                    index % 2 === 0 ? '#f3f3f3' : '#ffffff'; // Alternate row color
                  return `
