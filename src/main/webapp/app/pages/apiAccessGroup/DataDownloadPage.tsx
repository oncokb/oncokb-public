...
@observer
export default class DataDownloadPage extends React.Component<{
  routing: RouterStore;
  ...
  private canShowDataDownload() {
    return !!this.props.authenticationStore.account?.authorities?.includes(
      USER_AUTHORITY.ROLE_DATA_DOWNLOAD
    );
  }

  renderDataDownloadSection() {
    if (!this.canShowDataDownload()) {
      return (
        <div>
          <p>
            Downloadable data files require a license. Research licenses in an
            academic setting are free, and all other uses require a fee.
          </p>
          <p>
            Please review the <Link to={PAGE_ROUTE.TERMS}>terms of use</Link>{' '}
            before proceeding, then visit the{' '}
            <Link to={PAGE_ROUTE.REGISTER}>registration page</Link> to apply for
            a license.
          </p>
        </div>
      );
      ...
      <>
        ...
        this.dataAvailability.result.length > 0 ? (
          <>
            ...
            <p className="rounded">
              The transcript database serves OncoKB metadata included gene,
              transcript, sequence, etc. Only required for local installations
              that will utilize the /byGenomicChange and /byHGVSg endpoints
            </p>
          ...
          <Else>
            <Row>
              <Col>
                ...
                {this.renderDataDownloadSection()}
              </Col>
            </Row>
          </Else>
        </If>
