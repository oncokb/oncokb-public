...
import {MailTypeInfo, UserDTO, UserMailsDTO} from 'app/shared/api/generated/API';
...
export default class UserManagementPage extends React.Component<{
  ...
  constructor(props: Readonly<{ routing: RouterStore; authenticationStore: AuthenticationStore, match: match }>) {
    super(props);
    this.reactions.push(
      reaction(
  ...
  @computed
  get canSendEmail() {
    return this.selectedMailFrom && this.selectedMailType && this.selectedUserLogin;
  }

  @computed
  get selectedUser() {
    return _.chain(this.users.result).filter(user => user.login === this.selectedUserLogin).first().value();
  }
  ...
  private columns: SearchColumn<UserMailsDTO>[] = [
    ...
    },
    {
      ...
      onFilter: (data: UserMailsDTO, keyword) =>
        data.sentFrom
          ? filterByKeyword(data.sentFrom, keyword)
          : false,
      accessor: 'sentFrom'
    },
    {
      ...
      onFilter: (data: UserMailsDTO, keyword) =>
        data.sentBy
          ? filterByKeyword(data.sentBy, keyword)
          : false,
      accessor: 'sentBy'
    }
  ];
        ...
        <Then>
          ...
          <Row className={getSectionClassName()}>
            <Col className={classnames(...COMPONENT_PADDING)} lg={3} xs={12}>
              <Select
                value={this.selectedUserLogin ? {value: this.selectedUserLogin, label: this.selectedUserLogin} : null}
            ...
            <Col className={classnames(...COMPONENT_PADDING)} lg={4} xs={12}>
              <Select
                value={this.selectedMailFrom ? {value: this.selectedMailFrom, label: this.selectedMailFrom} : null}
                placeholder={`Select the email address you'd like to use`}
            ...
            <Col className={classnames(...COMPONENT_PADDING)} lg={3} xs={12}>
              <Select
                ...
                value={this.selectedMailType ? {value: this.selectedMailType.mailType, label: this.selectedMailType.description} : null}
            ...
            <Col className={classnames(...COMPONENT_PADDING)} lg={2} xs={12}>
              <LoadingButton
                variant="primary"
                type="submit"
