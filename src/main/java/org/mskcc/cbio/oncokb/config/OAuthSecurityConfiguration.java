    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        http
            .logout(logout -> logout
                ...
                // Sessions are persisted in JDBC via Spring Session (see HttpSessionConfiguration), so they survive pod restarts
                // and are shared across instances. invalidateHttpSession(true) removes the JDBC-backed session on logout.
                .logoutRequestMatcher(new AntPathRequestMatcher("/oauth2/logout", "GET"))
                .invalidateHttpSession(true)
                .clearAuthentication(true)
                // JSESSIONID is the default container cookie; SESSION is the cookie Spring Session uses.
