...
public class MailService {
    ...
    @Async
    public void sendNotActivatedUsersEmail(List<UserDTO> users){
        if (users == null || users.isEmpty()) {
            return;
        }
        Context context = new Context(Locale.US);
        ...
        try {
            sendEmail(applicationProperties.getEmailAddresses().getTechDevAddress(), applicationProperties.getEmailAddresses().getTechDevAddress(), null, "The list of expiring trial accounts", content, null, false, true);
            log.info("Sent email to User '{}'", applicationProperties.getEmailAddresses().getTechDevAddress());
        } catch (MailException | MessagingException e) {
            log.warn("Email could not be sent to user '{}'", applicationProperties.getEmailAddresses().getTechDevAddress(), e);
        }
